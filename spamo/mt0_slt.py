...
class MT0SLT(AbstractSLT):
    """
    mT0-based Sign Language Translation model for the Banglagov dataset.

    Uses the multilingual instruction-tuned ``bigscience/mt0-xl`` model
    (https://huggingface.co/bigscience/mt0-xl) as the text decoder backbone.
    Unlike the multilingual FlanT5 variant, this model does NOT use any
    multilingual in-context example dependencies; the prompt is a single
    fixed instruction and the visual features are prepended to it.
    """
    def __init__(
        self,
    ...
    def _apply_lora(self) -> None:
        ...
        print("LoRA adapter applied to mT0 model.")

    def _freeze_model(self) -> None:
        self.t5_model.eval()
        for params in self.t5_model.parameters():
            params.requires_grad = False
        print("mT0 model frozen.")
